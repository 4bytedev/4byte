...
class ArticleSaveObserver
{
    ...
    {
        $feedback = new Feedback('save', $articleSave->user_id, 'article:'.$articleSave->article_id, '', Carbon::now());
        $this->gorse->insertFeedback($feedback);
    }

    public function deleted(ArticleSave $articleSave)
    {
        $this->gorse->deleteFeedback('save', $articleSave->user_id, 'article:'.$articleSave->article_id);
    }
}
