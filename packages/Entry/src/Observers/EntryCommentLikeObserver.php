...
class EntryCommentLikeObserver
{
    ...
    {
        $feedback = new Feedback('like', (string) $entryCommentLike->user_id, "entry:{$entryCommentLike->entry_id}:comment:{$entryCommentLike->comment_id}", '', Carbon::now());
        $this->gorse->insertFeedback($feedback);
    }

    public function deleted(EntryCommentLike $entryCommentLike)
    {
        $this->gorse->deleteFeedback('like', (string) $entryCommentLike->user_id, "entry:{$entryCommentLike->entry_id}:comment:{$entryCommentLike->comment_id}");
    }
}
