...
class Settings extends Page implements HasForms
{
    ...
    {
        $data['siteSettings']['available_languages'] = collect($data['siteSettings']['available_languages'])
            ->pluck('available_language')
            ->toArray();

        $data['securitySettings']['allowed_emails'] = collect($data['securitySettings']['allowed_emails'])
            ->pluck('allowed_email')
            ->toArray();
    ...
    {
        $this->siteSettings['available_languages'] = collect($this->siteSettings['available_languages'])
            ->map(fn ($ip) => ['available_language' => $ip])
            ->toArray();
        $this->securitySettings['allowed_emails'] = collect($this->securitySettings['allowed_emails'])
        ...
        if (isset($data['seoSettings']['og_images'])) {
            ...
            if ($cond) {
                foreach ($oldImages as $old) {
                    if ($old && $old instanceof UploadedFile && Storage::disk('public')->exists($old)) {
                        Storage::disk('public')->delete($old);
                    }
                }
